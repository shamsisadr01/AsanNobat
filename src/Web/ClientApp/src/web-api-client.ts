...
export class BusinessClient {
    ...
    /**
     * Get a Business with details
     * @return OK
     */
    getBusiness(id: number): Promise<BusinessDetailsDto> {
        ...
        return this.http.fetch(url_, options_).then((_response: Response) => {
            return this.processGetBusiness(_response);
        });
    }

    protected processGetBusiness(response: Response): Promise<BusinessDetailsDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
            let result200: any = null;
            let resultData200 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result200 = BusinessDetailsDto.fromJS(resultData200);
            return result200;
            });
        } else if (status === 400) {
            return response.text().then((_responseText) => {
        ...
        return Promise.resolve<BusinessDetailsDto>(null as any);
    }
...
export class BusinessDetailsDto implements IBusinessDetailsDto {
    ...
    constructor(data?: IBusinessDetailsDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (this as any)[property] = (data as any)[property];
    ...
    static fromJS(data: any): BusinessDetailsDto {
        data = typeof data === 'object' ? data : {};
        let result = new BusinessDetailsDto();
        result.init(data);
        return result;
    }
